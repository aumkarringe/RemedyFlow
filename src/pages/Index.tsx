...
const Index = () => {
  ...
  const generateAIRemedies = async (query: string, datasetResults: Remedy[]) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/generate-remedies`,
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to generate AI remedies');
      }
  ...
  const handleRemedyCompare = async (remedies: string[]) => {
    setIsLoadingAI(true);
    setSubmittedQuery(`Comparing: ${remedies.join(", ")}`);
    setAiRemedies([]);
  ...
  const handleHealthJournalAnalysis = async (entries: any[]) => {
    setIsLoadingAI(true);
    setSubmittedQuery("Health Journal Analysis");
    setAiRemedies([]);
  ...
  const handleContraindicationCheck = async (items: { remedies: string[], medications: string[], conditions: string[] }) => {
    setIsLoadingAI(true);
    setSubmittedQuery("Safety Check");
    setAiRemedies([]);
  ...
  const handleWellnessPlanGeneration = async (goals: string[], duration: string) => {
    setIsLoadingAI(true);
    setSubmittedQuery(`${duration}-Day Wellness Plan`);
    setAiRemedies([]);
